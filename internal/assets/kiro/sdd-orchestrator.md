...
3. **Lifecycle receipt rule**: once START returns a lineage, preserve it. Before commit, stage every reviewed path without changing content or mode, then run native `gentle-ai review validate --gate pre-commit --cwd <repo> --lineage <known-lineage>`; before push, PR, or release, run the corresponding gate with the same exact `--lineage`. Never fall back to inventory discovery, launch a lens, repair authority, or create a new budget at a repeated gate.
