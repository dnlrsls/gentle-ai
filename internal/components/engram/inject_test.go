...
func TestInjectAntigravityInactivePluginPrefersLegacyGlobalCommand(t *testing.T) {
	home := t.TempDir()
	...
	pluginMCP := filepath.Join(pluginDir, "mcp_config.json")
	writeFile(t, global, `{"mcpServers":{"engram":{"command":"/working/engram"}}}`)
	writeFile(t, pluginMCP, `{"mcpServers":{"engram":{"command":"/stale/engram"}}}`)

	if _, err := Inject(home, antigravityAdapter()); err != nil {
		t.Fatal(err)
	}

	pluginConfig := readJSONFile(t, pluginMCP)
	assertNestedString(t, pluginConfig, "/working/engram", "mcpServers", "engram", "command")
	assertNestedStrings(t, pluginConfig, []string{"mcp", "--tools=agent"}, "mcpServers", "engram", "args")
	manifest, err := readImage(filepath.Join(pluginDir, "plugin.json"))
	if err != nil || !isActiveAntigravityManifest(manifest) {
		t.Fatalf("manifest active = %v, error = %v", isActiveAntigravityManifest(manifest), err)
	}
}
...
func TestInjectAntigravityRecoversWriteFailures(t *testing.T) {
	tests := []struct {
		...
		{"global post-replacement reread failure", "unknown", "", false, true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			actualWrite, actualRead, manifestCalls, globalReplaced := antigravityWriteFile, antigravityReadFile, 0, false
			antigravityWriteFile = func(path string, content []byte, mode os.FileMode) (filemerge.WriteResult, error) {
				...
				if path == global && !bytes.Equal(content, original) && tt.global != "" {
					...
					r, _ := actualWrite(path, content, mode)
					globalReplaced = true
				...
				return actualWrite(path, content, mode)
			}
			antigravityReadFile = func(path string) ([]byte, error) {
				if path == global && globalReplaced && tt.global == "unknown" {
					return nil, fmt.Errorf("global reread fail")
				}
				return actualRead(path)
			}
			t.Cleanup(func() {
				antigravityWriteFile = actualWrite
				antigravityReadFile = actualRead
			})
			...
			if tt.pluginOnly {
				...
				manifestImage, readErr := readImage(manifest)
				if readErr != nil || !isActiveAntigravityManifest(manifestImage) {
					t.Fatalf("manifest active = %v, error = %v", isActiveAntigravityManifest(manifestImage), readErr)
				}
				pluginConfig := readJSONFile(t, filepath.Join(filepath.Dir(manifest), "mcp_config.json"))
				assertNestedString(t, pluginConfig, "engram", "mcpServers", "engram", "command")
				assertNestedStrings(t, pluginConfig, []string{"mcp", "--tools=agent"}, "mcpServers", "engram", "args")
...
func TestInjectAntigravityManifestWriteFailureUsesSemanticActivity(t *testing.T) {
	tests := []struct {
		name, variant, manifest string
		wantPluginOnly          bool
	}{
		{"CLI invalid manifest restores global", "antigravity-cli", `{"name":`, false},
		{"desktop invalid manifest restores global", "antigravity-desktop", `{"name":`, false},
		{"CLI older valid manifest preserves plugin-only", "antigravity-cli", `{"name":"gentle-ai-engram","version":"0.0.1","custom":true}`, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			home := t.TempDir()
			mockEngramLookPath(t, "", "not found")
			dir := filepath.Join(home, ".gemini", tt.variant)
			if tt.variant == "antigravity-desktop" {
				if err := os.MkdirAll(dir, 0o755); err != nil {
					t.Fatal(err)
				}
			}
			global := filepath.Join(dir, "mcp_config.json")
			manifest := filepath.Join(dir, "plugins", "gentle-ai-engram", "plugin.json")
			pluginMCP := filepath.Join(filepath.Dir(manifest), "mcp_config.json")
			writeFile(t, global, `{"root":1,"mcpServers":{"engram":{"command":"engram"},"other":{"command":"other"}}}`)
			writeFile(t, manifest, tt.manifest)
			...
			globalConfig := readJSONFile(t, global)
			servers, ok := globalConfig["mcpServers"].(map[string]any)
			if !ok {
				t.Fatalf("global mcpServers has unexpected type: %T", globalConfig["mcpServers"])
			}
			_, globalActive := servers["engram"]
			if globalActive != !tt.wantPluginOnly || servers["other"] == nil || globalConfig["root"] != float64(1) {
				t.Fatalf("global registration state = %v, config = %v", globalActive, globalConfig)
			}
			...
			if active := isActiveAntigravityManifest(manifestImage); active != tt.wantPluginOnly {
				t.Fatalf("manifest active = %v, want %v; manifest = %s", active, tt.wantPluginOnly, manifestImage.data)
			}
			var manifestConfig map[string]any
			parseErr := json.Unmarshal(manifestImage.data, &manifestConfig)
			if tt.wantPluginOnly {
				if parseErr != nil || manifestConfig["name"] != "gentle-ai-engram" || manifestConfig["version"] != "0.0.1" || manifestConfig["custom"] != true {
					t.Fatalf("older manifest not preserved: config=%v err=%v", manifestConfig, parseErr)
				}
				if !strings.Contains(err.Error(), "plugin-only registration observed") {
					t.Fatalf("recovery error does not report plugin-only state: %v", err)
				}
			} else {
				if parseErr == nil {
					t.Fatalf("invalid manifest unexpectedly parsed: %v", manifestConfig)
				}
				if strings.Contains(err.Error(), "plugin-only registration observed") {
					t.Fatalf("recovery error incorrectly reports plugin-only state: %v", err)
				}
			}
		})
	}
